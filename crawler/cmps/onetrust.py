...
class OnetrustCMP(AbstractCMP):
    ...
    def scrape(self, url: str, visit: SiteVisit, webdriver: CBConsentCrawlerBrowser) -> Tuple[CrawlState, str]:
        ...
        self.logger.info("ONETRUST: Attempting Variant A (browser_id: %s)", browser.browser_id)
    
        result = webdriver.execute_in_IFrames(self._variantA_try_retrieve_ddid, timeout=5)
    
        if result:
            domain_url = result[0]
            dd_id = result[1]
            self.logger.info("ONETRUST: VARIANT A: OneTrust data domain url = %s, %s (browser_id: %s)", domain_url, dd_id, browser.browser_id)
    
            # Variant A, Part 2: Using the data domain ID, retrieve ruleset ID list
            rs_ids, state, report = self._variantA_try_retrieve_ruleset_id(domain_url, dd_id, webdriver)
            if state != CrawlState.SUCCESS:
                self.logger.error("FAILED to retrieve ruleset_id: %s (browser_id: %s)", state, browser.browser_id)
                return state, report
    
            self.logger.info("ONETRUST: VARIANT A: Found %s ruleset ids (browser_id: %s)", len(rs_ids), browser.browser_id)
            self.logger.debug("ONETRUST: VARIANT A: Retrieved ruleset ids %s", rs_ids, browser.browser_id)
    
            # Variant A, Part 3: For each ruleset id, retrieve cookie json
            cookie_count, state, report = self._variantA_get_and_parse_json(domain_url, dd_id, rs_ids, webdriver, visit)
            if state != CrawlState.SUCCESS:
                self.logger.error("FAILED to get and parse json with dd_id: %s (browser_id: %s)", dd_id, browser.browser_id)
                return state, report
    
            self.logger.info("ONETRUST: VARIANT A: Retrieved %s cookies (browser_id: %s)", cookie_count, browser.browser_id)
        else:
            # Variant B, Part 1: Obtain the javascript URL
    
            self.logger.info("ONETRUST: Attempting Variant B (browser_id: %s)", browser.browser_id)
    
            script_url = webdriver.execute_in_IFrames(self._variantB_try_retrieve_jsurl, timeout=5)
            if not script_url:
                report = "ONETRUST: Could not find a valid OneTrust CMP Variant on this URL."
                self.logger.error("%s (browser_id: %s): %s", report, browser.browser_id, result)
    
                return CrawlState.CMP_NOT_FOUND, report
            self.logger.info("ONETRUST: VARIANT B: Onetrust Javascript URL = %s (browser_id %s)", script_url, browser.browser_id)
    
            # Variant B, Part 2: Access the script and retrieve raw data from it
            data_dict, state, report = self._variantB_parse_script_for_object(script_url, webdriver)
            if state != CrawlState.SUCCESS or data_dict is None:
                self.logger.error("Failed with state %s: %s and data_dict %s", state, report, data_dict)
                return state, report
            self.logger.info("ONETRUST: VARIANT B: Successfully retrieved OneTrust Consent javascript object data. (browser_id %s)", browser.browser_id)
    
            # Variant B, Part 3: Extract the cookie values from raw data
            cookie_count, state, report = self._variantB_extract_cookies_from_dict(data_dict, browser, visit)
            if state != CrawlState.SUCCESS:
                self.logger.error("Failed in part3 with state %s: %s", state, report)
                return state, report
    
            self.logger.info("ONETRUST: VARIANT B: Retrieved %s cookies (browser_id: %s)", cookie_count, browser.browser_id)
    
        return CrawlState.SUCCESS, f"Extracted {cookie_count} cookie entries."
    
    
    def category_lookup_en(self, browser: Crawl, cat_name: str) -> CookieCategory:
        """
        ...
        if en_targeting_pattern.search(cat_name): return CookieCategory.ADVERTISING
            ...
            self.logger.warning("ONETRUST: %s not recognized by English patterns (browser_id: %s)", cat_name, browser.browser_id)
            return CookieCategory.UNRECOGNIZED
    
    
    def category_lookup_de(self, browser: Crawl, cat_name: str) -> CookieCategory:
        """
        Map english category name defined in the CMP to the internal representation.
        """
        if de_targeting_pattern.search(cat_name): return CookieCategory.ADVERTISING
            ...
            self.logger.warning("ONETRUST: '%s' not recognized by German patterns (browser_id: %s)", cat_name, browser.browser_id)
            return CookieCategory.UNRECOGNIZED
    
    
    def _exists_script_tag_with_ddid(self, driver: WebDriver, browser: Crawl) -> Union[bool, Tuple[str, str]]:
        """
            ...
            try:
                ...
                if (dd_id is not None) and (uuid_pattern.match(str(dd_id)) or str(dd_id) == "center-center-default-stack-global-ot"):
                    ...
                    if source_stub is None:
                        self.logger.warning("ONETRUST: VARIANT A: Found a script tag with the data-domain attribute, but no URL? Script ID: %s (browser_id %s)", dd_id, browser.browser_id)
                        continue
                    else:
                        for pat in base_patterns:
                            ...
                            self.logger.warning("ONETRUST: VARIANT A: Found a data-domain-script tag with unknown source URL: %s. Script ID: %s (browser_id: %s)", source_stub, dd_id, browser.browser_id)
    ...
    def _exists_script_tag_with_jsurl(self, driver, browser) -> Union[bool, str]:
        """
            ...
            try:
                ...
                if source:
                    ...
                    for p in variantB_patterns:
                        ...
                        if matchobj:
                            self.logger.info("ONETRUST: VARIANT B: Pattern found: %s (browser_id: %s)", p.pattern, browser.browser_id)
    ...
    def _variantA_try_retrieve_ddid(self, driver: WebDriver, browser: Crawl, timeout: int = 5) -> Optional[Tuple[str, str]]:
        """
        ...
        try:
            ...
            tup = cast(Tuple[str, str], wait.until(lambda x : self._exists_script_tag_with_ddid(driver=x, browser=browser)))
            # base_domain, dd_id
            return tup[0], tup[1]
        except TimeoutException:
            self.logger.info("ONETRUST: VARIANT A: Timeout on trying to retrieve data domain id value. (browser_id: %s)", browser.browser_id)
            return None
    
    
    def _variantA_try_retrieve_ruleset_id(self, domain_url: str, dd_id: str,
        ...
        try:
            ...
            if rulesets is None:
                self.logger.error(f"ONETRUST: VARIANT A: No valid 'RuleSet' element found on {target_url}")
                ...
                for r in rulesets:
                    ...
                    if "en" in languageset.values():
                        ...
                        self.logger.warning("ONETRUST: VARIANT A: Ruleset did not have a recognized language, defaulting to english. (browser_id: %s)", browser.crawl.browser_id)
                        ids.append(("en", r["Id"]))
    
            if len(ids) == 0:
                self.logger.error(f"ONETRUST: VARIANT A: No valid language ruleset found on {target_url}")
            ...
            self.logger.error(f"ONETRUST: VARIANT A: Key Error on {target_url} -- Details: {kex}")
            return [], CrawlState.PARSE_ERROR, f"ONETRUST: VARIANT A: Key Error on {target_url} -- Details: {kex}"
    
    
    def _variantA_get_and_parse_json(self, domain_url: str, dd_id: str, ruleset_ids: List[Tuple[str, str]], webdriver: CBConsentCrawlerBrowser, visit: SiteVisit) -> Tuple[int, CrawlState, str]:
        """
        ...
        for lang, i in ruleset_ids:
            ...
            if state != PageState.OK:
                self.logger.error("ONETRUST: VARIANT A: Failed to retrieve ruleset at: %s, (browser_id: %s)", curr_ruleset_url, browser.browser_id)
                self.logger.error("ONETRUST: VARIANT A: Details: %s -- %s (browser_id: %s)", state, f"State is {state}", browser.browser_id)
                continue
            ...
            try:
                ...
                if "DomainData" not in json_data:
                    self.logger.warning("ONETRUST: VARIANT A: Could not find \"DomainData\" attribute inside decoded JSON. (browser_id: %s)", browser.browser_id)
                    continue
                json_body = json_data["DomainData"]
    
                ## Language Detection
                if "Language" not in json_body:
                    self.logger.warning("ONETRUST: VARIANT A: Could not find \"Language\" attribute inside decoded JSON. (browser_id: %s)", browser.browser_id)
                    continue
                elif "Culture" not in json_body["Language"]:
                    self.logger.warning("ONETRUST: VARIANT A: Could not find \"Culture\" attribute inside decoded JSON. (browser_id: %s)", browser.browser_id)
                    continue
                    ...
                    self.logger.warning("ONETRUST: VARIANT A: Unrecognized language in ruleset: %s", json_body['Language']['Culture'], browser.browser_id)
                    self.logger.warning("ONETRUST: VARIANT A: Trying english anyways...")
                    cat_lookup = self.category_lookup_en
    
                ## Cookie Data extraction
                if "Groups" not in json_data["DomainData"]:
                    self.logger.warning("ONETRUST: VARIANT A: Could not find \"Groups\" attribute inside decoded JSON. (browser_id: %s)", browser.browser_id)
                    continue
    
                group_list = json_data["DomainData"]["Groups"]
                for g_contents in group_list:
                    if "GroupName" not in g_contents:
                        self.logger.warning("ONETRUST: VARIANT A: Could not find Category Name for group inside decoded JSON. (browser_id: %s)", browser.browser_id)
                        continue
                    ...
                    if "FirstPartyCookies" in g_contents:
                        ...
                        self.logger.warning("ONETRUST: VARIANT A: No First Party Cookies inside group for decoded JSON.")
                    ...
                    if "Hosts" in g_contents:
                        ...
                        pass
                        self.logger.warning("ONETRUST: VARIANT A: No Third Party Cookies inside group for decoded JSON.")
            except (AttributeError, KeyError) as ex:
                self.logger.error("ONETRUST: VARIANT A: Could not retrieve an expected attribute from json. (browser_id %s)", browser_id)
                self.logger.error("ONETRUST: VARIANT A: Details: %s -- %s (browser_id: %s)", type(ex), ex, browser_id)
            except json.JSONDecodeError as ex:
                self.logger.error("ONETRUST: VARIANT A: Failed to decode json file for ruleset : %s (browser_id %s)", curr_ruleset_url, browser_id)
                self.logger.error("ONETRUST: VARIANT A: Details: %s -- %s (browser_id: %s)", type(ex), ex, browser_id)
                continue
    ...
    def _variantB_try_retrieve_jsurl(self, driver: WebDriver, browser: Crawl, timeout: int = 5) -> Optional[str]:
        """
        ...
        try:
            wait = WebDriverWait(driver, timeout)
            return cast(str, wait.until(lambda x: self._exists_script_tag_with_jsurl(driver=x, browser=browser)))
        except TimeoutException:
            self.logger.info("ONETRUST: VARIANT B: Timeout on trying to retrieve javascript link. (browser_id: %s)", browser.browser_id)
            return None
    
    
    def _variantB_parse_script_for_object(self, script_url: str, webdriver: CBConsentCrawlerBrowser) -> Tuple[Optional[Dict[str, Any]], CrawlState, str]:
        """
        ...
        try:
            if matchobj:
                ...
                self.logger.error(f"Please report this to the developer. Group string: {group_string}")
    ...
    def _variantB_extract_cookies_from_dict(self, consent_data: Dict[str, Any], browser: Crawl, visit: SiteVisit) -> Tuple[int, CrawlState, str]:
        """
        ...
        try:
            ...
            for g_contents in g_data:
                ...
                try:
                    ...
                    if len(langproplist) > 0:
                        ...
                        cat_id = self.category_lookup_en(browser, cat_name)
                        if cat_id == CookieCategory.UNRECOGNIZED:
                            cat_id = self.category_lookup_de(browser, cat_name)
                    ...
                    self.logger.warning("ONETRUST: Unable to find category name. Attempting cookie retrieval anyways... (browser_id: %s)", browser.browser_id)
            ...
            self.logger.error(f"ONETRUST: VARIANT B: Could not retrieve an expected attribute from consent data dict. -- {type(ex)} - {ex}")
        ...
        if cookie_count == 0:
            self.logger.warning("ONETRUST: VARIANT B: Consent Platform Script contained zero cookies!")
            return 0, CrawlState.NO_COOKIES, "ONETRUST: VARIANT B: Consent Platform Script contained zero cookies!"
        else:
            self.logger.info(f"ONETRUST: VARIANT B: Successfully retrieved {cookie_count} cookies.")
